...
describe('Identities: Restore from local', () => {
  ...
  it('restores the identity details', async () => {
    ...
    expect(app.text()).toContain(localCacheAdapterData['abc-123'].name)
  })

  it('restores the items', async () => {
    expect(app.text()).toContain(localCacheAdapterData['abc-123'].items[0].title)
  })

  it('restores the feeds', async () => {
    await app.click('[aria-label="Feeds"]')

    expect(app.text()).toContain(localCacheAdapterData['abc-123'].feeds[0].name)
  })
