      ...
      <div v-else>
        <div v-if="feed">
          ...
          <div v-if="sortedItems && sortedItems.length">
            <button @click="app.subscribe(feed, sortedItems)">
              Subscribe
            </button>
...
var XML_URL = /https?:\/\/[a-zA-Z0-9./]+\.xml([?a-zA-Z0-9_=-]+)?/g
var LINK_TAG = /<link[^>]+application\/rss[^>]+>/g
...
function checkURLAndPossibilitiesForData (app, feed, url, done) {
  ...
  getContent(proxy, url, function (err, data) {
    ...
    if (isRSS(data)) {
      ...
      var moreFeeds = (data.match(LINK_TAG) || []).map(function (tag) {
